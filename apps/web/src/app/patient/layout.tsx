...
const NAV_ITEMS = [
  ...
  { href: "/patient/intake", label: "Intake & Screening", icon: ClipboardList },
  { href: "/patient/records", label: "Health Records", icon: Activity },
  { href: "/patient/appointments", label: "Appointments", icon: Calendar },
  { href: "/patient/messages", label: "Messages", icon: MessageSquare },
  { href: "/patient/care-call", label: "Virtual Care", icon: Video },
  { href: "/patient/health-links", label: "Health Links", icon: QrCode },
];
  ...
  return (
    <div className="flex min-h-screen flex-col bg-[var(--paper)] text-[var(--ink)] md:flex-row transition-colors">
      ...
      <aside className="hidden md:flex w-64 flex-col border-r border-[var(--line)] bg-[var(--paper-raised)] p-4 justify-between shadow-xs">
        <div className="space-y-6">
          ...
          <div className="px-3 py-2 rounded border border-[var(--line)] bg-[var(--paper-sunken)] flex items-center justify-between font-mono text-[11px]">
            <span className="text-[var(--ink-muted)] uppercase tracking-wider">PATIENT SENTINEL</span>
            <span className="flex items-center gap-1.5 text-emerald-500 font-semibold text-[10px]">
        ...
        <div className="border-t border-[var(--line)] pt-4 space-y-2">
          <div className="flex items-center gap-3 px-2 py-1 text-xs text-[var(--ink-muted)]">
            ...
            <div>
              <p className="font-semibold text-[var(--ink)]">Sarah Johnson</p>
              <p className="font-mono text-[10px] text-[var(--ink-faint)]">MRN: 948204 · FHIR R4</p>
            </div>
          </div>
          <Link href="/signin">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <nav className="md:hidden fixed bottom-0 left-0 right-0 border-t border-[var(--line)] bg-[var(--paper-raised)]/95 backdrop-blur-md flex justify-around p-2 z-50">
          {NAV_ITEMS.slice(0, 5).map((item) => {
            const Icon = item.icon;
            const isActive = pathname.startsWith(item.href);
            return (
              <Link key={item.href} href={item.href} className="flex flex-col items-center py-1 text-[10px] font-mono text-[var(--ink-faint)]">
                <Icon className={cn("size-4 mb-0.5", isActive && "text-[var(--ink)]")} />
                <span className={cn(isActive && "text-[var(--ink)] font-semibold")}>{item.label.split(" ")[0]}</span>
              </Link>
            );
          })}
        </nav>
